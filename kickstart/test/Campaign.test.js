...
beforeEach(async () => {
  ...
  // Extract the first element from the array result, and assign as campaignAddress.
...
describe('Campaigns', () => {
  ...
  it('processes request', async () => {
    ...
    await campaign.methods
      .createRequest('Description', web3.utils.toWei(value.toString(), 'ether'), accounts[1])
      .send({ from: accounts[0], gas: 1000000 });
    ...
    const balance = await getBalanceInEther(accounts[1]);

    assert(balance > 104);
    assert.equal(value, balance - initialBalance);
  });
});

const getBalanceInEther = async (address) => {
